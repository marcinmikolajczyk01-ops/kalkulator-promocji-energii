...
function run(opts) {
  const o = Object.assign({ prof:'real', kwh:2504, rdnAdj:0, kb:60, handl:35, bat:0, cyc:1,
    ...
    dt:'all', months:allM, hStart:1, hEnd:24 }, opts);
...
// T3: Dane ROCZNE niezależne od okna promocji (magazyn = cecha instalacji klienta).
// Zmiana okna NIE może ruszać rocznego poboru / autokonsumpcji / eksportu.
{
  const bat = { prof:'real', pvKwp:5, pvExport:true, pvExportFee:25, pvMismatch:25, bat:10, cyc:1 };
  const apr = run(Object.assign({}, bat, { dt:'all', months:[4],  hStart:1, hEnd:24 }));
  const may = run(Object.assign({}, bat, { dt:'all', months:[5],  hStart:1, hEnd:24 }));
  const eve = run(Object.assign({}, bat, { dt:'all', months:allM, hStart:18, hEnd:21 }));
  const yr  = run(Object.assign({}, bat, { dt:'all', months:allM, hStart:1, hEnd:24 }));
  const same = (a,b) => Math.abs(a-b) < 1;
  console.log('T3  Dane roczne niezależne od okna (z magazynem 10 kWh)');
  console.log('    pobrana/rok :', [apr,may,eve,yr].map(r=>r.realImportAnnual.toFixed(0)).join(' / '), '(kwi/maj/wiecz/rok)');
  console.log('    oddana/rok  :', [apr,may,eve,yr].map(r=>r.realExportAnnual.toFixed(0)).join(' / '));
  console.log('    autokons/rok:', [apr,may,eve,yr].map(r=>r.pvAutoTotal.toFixed(0)).join(' / '), 'kWh');
  console.log('    =>', check(
    same(apr.realImportAnnual,yr.realImportAnnual) && same(may.realImportAnnual,yr.realImportAnnual) && same(eve.realImportAnnual,yr.realImportAnnual) &&
    same(apr.realExportAnnual,yr.realExportAnnual) && same(eve.realExportAnnual,yr.realExportAnnual) &&
    same(apr.pvAutoTotal,yr.pvAutoTotal) && same(eve.pvAutoTotal,yr.pvAutoTotal)
  ), '\n');
}

// T4: Realistyczny model — w TANIM oknie z eksportem magazyn może PODNIEŚĆ koszt NEXBE
// (klient magazynuje swoją nadwyżkę PV zamiast ją eksportować → mniej kredytu eksportu dla NEXBE).
{
  const win = { prof:'real', pvKwp:5, pvExport:true, pvExportFee:25, dt:'wknd', months:allM, hStart:11, hEnd:16 };
  const nb  = run(Object.assign({}, win, { bat:0 }));
  const mag = run(Object.assign({}, win, { bat:10, cyc:1 }));
  console.log('T4  Tanie okno + eksport — magazyn może podnieść koszt NEXBE (realistycznie)');
  console.log('    bez magazynu kostReal =', z(nb.kostReal), '| z magazynem =', z(mag.kostReal),
              '| Δ =', z(mag.kostReal-nb.kostReal));
  console.log('    =>', check(mag.kostReal > nb.kostReal - 0.01), '\n'); // koszt nie spada (rośnie lub ~równy)
}
...
// T6: Dane ROCZNE niezależne od okna — szeroki przemiał (oba profile × PV × eksport).
// Roczny pobór i eksport muszą być identyczne dla dowolnego okna (cecha instalacji).
{
  const windows = [
    { dt:'all',  months:allM, hStart:1,  hEnd:24 },
    { dt:'wknd', months:allM, hStart:11, hEnd:16 },
    { dt:'all',  months:[4],  hStart:1,  hEnd:24 },
    { dt:'all',  months:allM, hStart:18, hEnd:21 },
  ];
  console.log('T6  Dane roczne niezależne od okna — oba profile × PV × eksport (magazyn 10 kWh)');
  let maxSpread = 0, worstNm = '';
  for (const prof of ['real', 'slp'])
    for (const exp of [false, true])
      for (const pv of [3, 5]) {
        const base = { prof, kwh:2500, pvKwp:pv, pvExport:exp, pvExportFee:25, bat:10, cyc:1 };
        const imp = windows.map(w => run(Object.assign({}, base, w)).realImportAnnual);
        const exr = windows.map(w => run(Object.assign({}, base, w)).realExportAnnual);
        const spread = Math.max(Math.max(...imp)-Math.min(...imp), Math.max(...exr)-Math.min(...exr));
        if (spread > maxSpread) { maxSpread = spread; worstNm = `${prof}, PV ${pv}, eksport ${exp?'ON':'OFF'}`; }
      }
  console.log('    największy rozrzut rocznych (import/eksport) między oknami =', maxSpread.toFixed(2), 'kWh @', worstNm);
  console.log('    =>', check(maxSpread < 1), '\n');   // praktycznie zero — roczne nie zależą od okna
}
